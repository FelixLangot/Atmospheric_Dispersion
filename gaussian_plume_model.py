...
if output == PLAN_VIEW:
   ...
   plt.figure()
   plt.ion()
   x_slice = config_var['xpos']
   y_slice = config_var['ypos']
   ...
   if freq == 'mois':
     ...
     toplot.isel(x = x_slice, y=y_slice)['concs'].plot.line(x='month') 
     plt.xlabel('time (months)');
   elif freq == 'jour':
     toplot = cts.groupby('time.day').mean() 
     toplot.isel(x = x_slice, y=y_slice)['concs'].plot.line(x='day') 
     plt.xlabel('time (days)');
   plt.ylabel('Mass loading (mu g m^{-3})');
   plt.title(stability_str +'\n' + wind_dir_str);
